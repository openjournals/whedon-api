...
describe GitHub do
  ...
  context "for a single reviewer" do
    ...
    it "should know that a reviewer not involved with the review doesn't need a reminder" do
      expect(subject.needs_reminder?('openjournals/joss-reviews-testing', 938, '@human')).to be_falsey
    end

    it "should know that a reviewer involved with a review does need a reminder" do
      expect(subject.needs_reminder?('openjournals/joss-reviews-testing', 938, '@rlbarter')).to be_truthy
    end
  end
  ...
  context "for multiple reviewers" do
    ...
    it "should know that a reviewer not involved with the review doesn't need a reminder" do
      expect(subject.needs_reminder?('openjournals/joss-reviews-testing', 1203, '@human')).to be_falsey
    end

    it "should know that a reviewer involved with a review does need a reminder" do
      expect(subject.needs_reminder?('openjournals/joss-reviews-testing', 1203, '@stuartcampbell')).to be_truthy
    end
